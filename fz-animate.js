  ...
  animeLoadPromise = import(ANIME_JS_CDN_URL)
    .then((module) => {
      if (
        ...
        typeof module.onScroll === "function"
      ) {
        animeLibrary = module;
        ...
        throw new Error(
          "Anime.js module loaded but required functions not found"
        );
      }
    })
    .catch((error) => {
...
class FrenzyAnimate extends HTMLElement {
  ...
  async #initialize() {
    try {
      ...
      if (this.#target && this.#animationOptions) {
        ...
        console.warn("Missing target or options:", {
          target: this.#target,
  ...
  #getAnimationProps() {
    ...
    const isScrollBased =
      this.hasAttribute("scroll-container") ||
      this.hasAttribute("scroll-target");
  ...
  #getScrollOptions() {
    ...
    if (enterAttr) {
      scrollOptions.enter = enterAttr.includes("%")
        ? enterAttr
        : parseFloat(enterAttr);
    } else {
    ...
    if (leaveAttr) {
      scrollOptions.leave = leaveAttr.includes("%")
        ? leaveAttr
        : parseFloat(leaveAttr);
    } else {
    ...
    if (this.hasAttribute("scroll-horizontal")) {
      scrollOptions.horizontal =
        this.getAttribute("scroll-horizontal") !== "false";
    }
